...
export default function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  // Zustand store
  const { sendOtp, verifyOtp, otpSending, otpError, setOtpError } =
    useAuthStore();
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    const result = await sendOtp({
      ...
      // userType is always 'candidate' inside the store
    });
  ...
  const handleGoogleSignIn = () => {
    // Extra logic after Google Sign-In if needed
  };
  ...
  const handleOtpKeyDown = () => {
    // Focus handling is managed inside OtpVerificationForm
  };
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    const result = await verifyOtp(otpCode); // no userType param
