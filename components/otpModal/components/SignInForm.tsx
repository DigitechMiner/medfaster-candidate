...
import { detectContactType } from "@/app/utils/otp";
import { UserType } from "@/app/stores/types";

type AuthTab = "Sign Up" | "Login";

interface SignInFormProps {
  // UI tab state (Sign Up / Login)
  activeTab: AuthTab;
        ...
        if (userInfoResponse.ok) {
          ...
          if (sendOtp && email) {
            try {
              ...
              // For candidate-only site, userType will always be 'candidate'
  ...
  return (
    <>
      ...
      <div className="flex gap-2 mb-6 border rounded-lg">
        {(["Sign Up", "Login"] as AuthTab[]).map((tab) => (
          <button
            key={tab}
      ...
      <form onSubmit={onSendOTP} className="space-y-4">
        <div>
          ...
          <div
            className={`flex overflow-hidden items-center border border-gray-200 rounded-lg focus-within:ring-2 focus-within:ring-[#F4781B] focus-within:border-transparent bg-white ${
              !isPhoneNumber ? "px-0" : ""
            }`}
          >
            {isPhoneNumber && (
            ...
            <input
              ...
              placeholder={
                isPhoneNumber ? "(555) 000-0000" : "Enter email or phone number"
              }
              value={contactValue}
              onChange={(e) => handleInputChange(e.target.value)}
              disabled={otpSending}
              className={`${
                isPhoneNumber ? "flex-1 px-2" : "w-full px-4"
              } py-3 border-0 focus:outline-none text-sm bg-transparent disabled:opacity-50`}
              autoCapitalize="none"
              required
            />
          </div>
      ...
      >
        {googleLoading ? (
          <>
            ...
            <span className="text-[#252B37] font-medium text-sm">
              Connecting...
            </span>
          </>
        ) : (
